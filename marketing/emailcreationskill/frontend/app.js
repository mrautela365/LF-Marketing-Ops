...
function _openAudienceStream(jobId, streamSessionId, handlers) {
  ...
  es.onmessage = (event) => {
    ...
    if (msg.type === "output" && msg.text) { handlers.onOutput && handlers.onOutput(msg.text); return; }
    if (msg.type === "complete") { es.close(); handlers.onComplete && handlers.onComplete(msg); return; }
