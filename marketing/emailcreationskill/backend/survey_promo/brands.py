"""
Static brand/design-system constants for the Survey/Report promo tab.

Encodes the design-system table from the project brief: header logo, hero style,
CTA color, sender identity, and the type-level rules (surveys carry the
Privacy/Visibility/Questions legal block, reports carry the share block instead).

routes.py pins FIXED_BRAND_ID = "lf_research"; the other entries are kept so the
picker can be re-enabled without re-deriving them.
"""
