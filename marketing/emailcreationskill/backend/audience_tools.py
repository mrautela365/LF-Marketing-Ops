...
def start_plan_job(event_url: str, prescraped: dict | None = None) -> str:
    ...
    if LITELLM_BASE_URL and LITELLM_API_KEY:
        log.info(f"[AUDIENCE] plan job {job_id[:8]} — LiteLLM mode — url={event_url!r} reuse_scrape={bool(prescraped)}")
...
def start_build_job(event_url: str, plan: str = "", qa: str = "", prescraped: dict | None = None) -> str:
    ...
    if LITELLM_BASE_URL and LITELLM_API_KEY:
        if plan:
            ...
            log.info(f"[AUDIENCE] build job {job_id[:8]} — two-phase (no plan provided) — url={event_url!r} reuse_scrape={bool(prescraped)}")
