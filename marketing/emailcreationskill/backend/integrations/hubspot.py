...
def lookup_brand_history(brand_name: str, email_type_hint: str = None) -> dict:
    ...
    return {
        ...
        "subscription_type_id": to_obj.get("subscriptionId"),
    }
...
def get_email_details(email_id: str) -> dict:
    """
    Fetch email details including subject, preview, and HTML content.
    Used for content analysis and optimization.
    """
    log = logging.getLogger("email-staging")
    try:
        ...
        # Try to get preview text from content widgets
        content = email.get("content") or {}
        top_widgets = content.get("widgets") or {}
        pt_body = (top_widgets.get("preview_text") or {}).get("body") or {}
        preview = pt_body.get("value", "")

        # Extract HTML body - get the raw HTML from flexAreas
        html_parts = []
            ...
            for section in (area.get("sections") or []):
                for col in (section.get("columns") or []):
                    for widget_id in (col.get("widgets") or []):
                        widget = top_widgets.get(widget_id) or {}
                        # Extract HTML from rich text widget
                        if widget.get("type") == "text" or "html" in widget:
                            html = widget.get("body", {}).get("html", "")
                            if html:
        ...
        log.warning(f"get_email_details({email_id!r}) failed: {e}")
        return {
            "subject": "",
            "preview": "",
            "html": "",
...
def create_ab_variation(email_id: str, variation_name: str) -> dict:
    """
    Create a real HubSpot A/B test variation of an existing marketing email.

    Per https://developers.hubspot.com/docs/api-reference/legacy/marketing/marketing-emails/create-ab-variation:
      POST /marketing/v3/emails/ab-test/create-variation
      Body: {"contentId": "<email_id>", "variationName": "<name>"}

    The response is the new variation's own PublicEmail object — its "id" IS the new
    variant email's id (there is no separate ab-test id). HubSpot links the variation
    to the parent internally and sets state to DRAFT_AB_VARIANT. If an active variation
    already exists for this parent, HubSpot returns the existing one instead of creating
    a new one (per HubSpot's documented behavior).

    Args:
        email_id: The parent email ID to create a variation for (contentId)
        variation_name: Name for the new variation (variationName)

    Returns:
        {
            "status": "success" or "error",
            "variant_id": "...",   # the new variation's own email id
            "parent_email_id": "...",
            "state": "..."
        }
    """
    log = logging.getLogger("email-staging")
    try:
        result = _post("/marketing/v3/emails/ab-test/create-variation", {
            "contentId": str(email_id),
        ...
        if not variant_id:
            log.error(f"create_ab_variation({email_id}): No id returned. Response: {result}")
            return {
                "status": "error",
                "error": f"No email id returned from API: {result}",
                "variant_id": None
            }

        log.info(
            f"✅ Created A/B variation {variant_id} (state={result.get('state')}) "
            f"from parent {email_id}"
        )
        ...
        log.error(f"create_ab_variation({email_id}) failed: {e}")
        return {
            "status": "error",
            "error": str(e),
            "variant_id": None
