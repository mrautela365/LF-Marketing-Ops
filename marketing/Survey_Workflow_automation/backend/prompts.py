...
AUDIENCE_SYSTEM = """You help build HubSpot marketing-list audience segments for the \
Linux Foundation's research/events team from a short free-text description. You \
parse the request, propose a segment plan, and STOP — a human reviews and edits the \
plan before anything is created in HubSpot. Never invent locations, events, or \
suppression rules that are not stated or clearly implied by the request text.

Output ONLY a single JSON object matching the requested schema. No markdown fences, \
no commentary before or after the JSON."""

AUDIENCE_PLANNING_PROMPT_TEMPLATE = """Parse this free-text audience request into a \
structured segment plan.

Example request: "All mailable contacts located in Seoul, set up for mailing — this \
is for OSS Korea and MCP Dev Summit Seoul." -> this combines a LOCATION (Seoul) with \
NAMED EVENTS (OSS Korea, MCP Dev Summit Seoul) used for CONTEXT/naming only — the \
events do not restrict the list to past registrants of those events unless the \
request explicitly says so (e.g. "who registered for X" or "past attendees of X").

Rules:
- LOCATION(s): extract city (+ country/region only if the request is broad about \
region, e.g. "APAC" or "Korea" with no city) — default to city-only matching.
- NAMED EVENT(s): extract any named events/conferences mentioned, for context and \
list naming. Do NOT treat them as a registrant filter unless the request explicitly \
asks for past registrants/attendees of that event.
- Propose a clear "list_name" for the master/inclusion list (e.g. "Seoul - Mailable \
Contacts - OSS Korea / MCP Dev Summit Seoul").
- Decide "apply_suppression": true unless the request explicitly says this is a \
single-signal/building-block list that should skip suppression hygiene. When true, \
propose "suppression_search_terms" — short strings to search existing HubSpot lists \
by name for standard hygiene suppressions (e.g. ["opt-out", "gdpr", "suppression", \
"master exclusion"]).
- Write "plan_summary" as a short, plain-English paragraph a marketer can read and \
approve or correct — state the location(s), the named events for context, whether \
suppression will be applied, and the proposed list name.
- If anything is genuinely ambiguous and blocks building (e.g. request says a region \
with multiple plausible cities, or doesn't say whether to include past registrants), \
add an entry to "open_questions" with 2-4 concrete "options" the user can pick from. \
Only add a question if it truly blocks building — do not ask questions you can \
...
{{
  ...
  "named_events": ["...", ...],
  "list_name": "...",
  "apply_suppression": true/false,
  "suppression_search_terms": ["...", ...],
  "plan_summary": "...",
  "open_questions": [{{"question": "...", "why_it_blocks": "...", "options": ["...", ...]}}]
}}
...
def build_audience_planning_prompt(description: str, qa: str = "") -> str:
    qa_block = (
        f"The user already answered these clarifying questions — incorporate the "
        f"answers and do not re-ask them:\n{qa}"
        if qa else
        "No prior clarifying answers yet — this is the first planning pass."
    )
    return AUDIENCE_PLANNING_PROMPT_TEMPLATE.format(qa_block=qa_block, description=description)
