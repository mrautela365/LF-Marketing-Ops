"""
/setup routes — credential management web page and save endpoint.
"""
...
from ..models.hubspot import ApiResponse, SaveCredentialsRequest
...
async def setup_page() -> str:
    """Serve the credential setup HTML page."""
...
<title>Setup — HubSpot Form Manager</title>
<link rel="stylesheet" href="/static/style.css">
</head>
<body class="setup-page">
<div class="setup-container">
  <h1>⚙️ Credential Setup</h1>
  <p class="subtitle">Configure your API credentials to connect to HubSpot and Asana.</p>
  ...
  <form id="setup-form">
    <div class="field-group">
      ...
      <small>
        ...
        The app needs scopes: <code>forms</code>, <code>communication-preferences</code>,
        <code>automation</code>, <code>crm.lists.read</code>, <code>content</code>.
      </small>
    </div>

    <div class="field-group">
      <label for="asana_pat">
        Asana Personal Access Token <span class="optional">(optional — only needed for Asana pre-fill)</span>
      </label>
      <input type="password" id="asana_pat" name="asana_pat"
             placeholder="1/xxxxxxxxxxxx..."
             autocomplete="off">
      <small>
        Get this from <a href="https://app.asana.com/0/my-apps" target="_blank">Asana → My Profile → Apps → Personal Access Tokens</a>.
      </small>
    </div>

    <div class="form-actions">
      <button type="submit" class="btn-primary">Save Credentials</button>
      {"<a href='/' class='btn-secondary'>← Back to Form Manager</a>" if is_hubspot_configured() else ""}
    </div>
...
document.getElementById('setup-form').addEventListener('submit', async (e) => {{
  ...
  try {{
    ...
    if (data.success) {{
      ...
      result.innerHTML = '✅ Credentials saved! <a href="/">Open Form Manager →</a>';
    }} else {{
